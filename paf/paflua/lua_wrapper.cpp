#include "paflua.h"
#include "lua_subclass_invoker.h"
#include "lua_call_back.h"
#include "lua_call_back.mh"
...
static void* FunctionInvokerToLightUserData(pafcore::FunctionInvoker invoker)
{
	return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(invoker));
}

static pafcore::FunctionInvoker LightUserDataToFunctionInvoker(const void* p)
{
	return reinterpret_cast<pafcore::FunctionInvoker>(reinterpret_cast<uintptr_t>(p));
}
...
pafcore::ErrorCode GetPrimitiveOrEnum(lua_State *L, pafcore::Variant* variant)
{
	if (variant->m_type->isPrimitive())
	{
		...
		{
			pafcore::PrimitiveType* primitiveType = static_cast<pafcore::PrimitiveType*>(variant->m_type);
			if (pafcore::char_type == primitiveType->m_typeKind)
			{
	...
	case LUA_TSTRING:
		{
			...
			if(value->m_type->isPrimitive() && value->byValue())
			{
				...
				if(pafcore::float_type == primitiveType->m_typeKind ||
					pafcore::double_type == primitiveType->m_typeKind ||
					pafcore::long_double_type == primitiveType->m_typeKind)
				{
...
int FunctionInvoker_Closure(lua_State *L)
{
	...
	pafcore::FunctionInvoker invoker = LightUserDataToFunctionInvoker(p);
	int numArgs = lua_gettop(L);
	return InvokeFunction(L, invoker, numArgs, 1);
}
...
int Variant_Len(lua_State *L)
{
	...
	if (pafcore::void_instance == variant->m_type->m_kind && variant->byValue())
	{
...
//		if (variant)
//		{
//			if (pafcore::object_instance == variant->m_type->m_kind &&
...
pafcore::ErrorCode Variant_Index_Identifier(lua_State *L, pafcore::Variant* variant, const char *name)
{
	...
	case pafcore::class_type:
		{
			...
			if(0 != member)
			{
				...
				case pafcore::static_method:
					lua_pushlightuserdata(L, FunctionInvokerToLightUserData(static_cast<pafcore::StaticMethod*>(member)->m_invoker));
	...
	if(0 != member)
	{
		...
		case pafcore::instance_property:
			if (static_cast<pafcore::InstanceProperty*>(member)->isArray())
			{
				return MakeInstanceArrayProperty(L, variant, static_cast<pafcore::InstanceProperty*>(member));
			}
			else if (static_cast<pafcore::InstanceProperty*>(member)->isMap())
			{
		...
		case pafcore::instance_method:
			lua_pushlightuserdata(L, FunctionInvokerToLightUserData(static_cast<pafcore::InstanceMethod*>(member)->m_invoker));
			lua_pushcclosure(L, FunctionInvoker_Closure, 1);
			return pafcore::s_ok;
		case pafcore::static_method:
			lua_pushlightuserdata(L, FunctionInvokerToLightUserData(static_cast<pafcore::StaticMethod*>(member)->m_invoker));
	...
	else if (name[0] == '_')
	{
		...
		case 'C':
			...
			{
				if (pafcore::void_type == variant->m_type->m_kind ||
					pafcore::primitive_type == variant->m_type->m_kind ||
					pafcore::enum_type == variant->m_type->m_kind ||
					pafcore::class_type == variant->m_type->m_kind)
				{
		...
		case 'D':
			...
			{
				if (pafcore::class_type == variant->m_type->m_kind)
				{
		...
		case 'a':
			...
			{
				if (pafcore::value_instance == variant->m_type->m_kind &&
...
SubscriptKind Variant_ParseSubscript(size_t& num, const char*& str, lua_State *L)
{
	int index = 2;
	int type = lua_type(L, index);
	switch (type)
	...
	case LUA_TUSERDATA:{
		...
		if (variant)
		{
			...
			{
				pafcore::PrimitiveType* primitiveType = static_cast<pafcore::PrimitiveType*>(variant->m_type);
				if (pafcore::string_type == primitiveType->m_typeKind)
				{
...
int Variant_Index(lua_State *L) 
{
	size_t num;
	const char* str;
	SubscriptKind sc = Variant_ParseSubscript(num, str, L);
	...
	else
	{
		...
		{
			errorCode = Variant_Index_Identifier(L, variant, str);
		}
		else
		{
...
int Variant_NewIndex(lua_State *L) 
{
	...
	{
		errorCode = Variant_NewIndex_Identifier(L, variant, str);
	}
	else
	{
		errorCode = pafcore::e_invalid_subscript_type;
