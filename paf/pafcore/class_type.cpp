...
#include "class_type.mh"
#include "class_type.ic"
#include "class_type.mc"
#include "variant.h"
...
static Metadata* FindMetadataByName(Metadata** members, size_t memberCount, const char* name)
{
	if (nullptr == members || 0 == memberCount)
	{
		return nullptr;
	}
	Metadata dummy(name);
	Metadata* key = &dummy;
	Metadata** it = std::lower_bound(members, members + memberCount, key, CompareMetaDataPtrByName());
	if (members + memberCount != it && strcmp(name, (*it)->m_name) == 0)
	{
		return *it;
	}
	return nullptr;
...
Metadata* ClassType::_findMember_(const char* name, bool includeBaseClasses)
{
	...
	if(includeBaseClasses)
	{
		for(size_t i = 0; i < m_baseClassCount; ++i)
		{
			member = m_baseClasses[i].m_type->_findMember_(name, includeBaseClasses);
			if(nullptr != member)
			{
				return member;
			}
...
Metadata* ClassType::findMember(const char* name, bool includeBaseClasses)
{
	...
	if (nullptr == member)
	{
		member = _findMember_(name, includeBaseClasses);
		if (nullptr != member)
		{
			if (m_resolvedMembers.empty())
			{
				const uint32_t initial_capacity = 16;
				m_resolvedMembers.reserve(initial_capacity);
			}
			m_resolvedMembers.insert(member);
		}
	}
	return member;
}
...
Metadata* ClassType::findClassMember(const char* name, bool includeBaseClasses, bool typeAliasToType)
{
	...
	else if(includeBaseClasses)
	{
		for (size_t i = 0; i < m_baseClassCount; ++i)
		{
			member = m_baseClasses[i].m_type->findClassMember(name, includeBaseClasses, typeAliasToType);
			if (nullptr != member)
			{
				break;
			}
