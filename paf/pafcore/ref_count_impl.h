...
inline long_t Atomic_Increment(std::atomic<long_t>& addend)
{
	return addend.fetch_add(1, std::memory_order_relaxed) + 1;
}

inline long_t Atomic_Decrement(std::atomic<long_t>& addend)
{
	return addend.fetch_sub(1, std::memory_order_acq_rel) - 1;
}
